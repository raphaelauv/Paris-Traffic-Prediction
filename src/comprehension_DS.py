...
def sensor_with_all_data_AllYearsNot_NULL_notNov2017():
	return 'SELECT * FROM test WHERE taux_occ IS NOT NULL AND debit IS NOT NULL AND NOT (year=2017 AND numberWeek IN (44,45,46,47,48)) ' #ORDER BY year

def sensor_with_all_data_Not_NULL_Nov2017():
	return 'SELECT * FROM test WHERE taux_occ IS NOT NULL AND debit IS NOT NULL AND year=2017 AND numberWeek IN (44,45,46,47,48) '

#Dinf list of all sensors who worked all year
def sensor_with_all_data_AllYears_():
